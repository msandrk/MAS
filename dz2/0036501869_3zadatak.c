...
/*
    ...
    frequencies.
*/
void calculateFrequency(char const *filename, double *freqs){
    FILE *file = fopen(filename, "rb");
    int counts[NUM_OF_GROUPS] = { 0 };
    int width = 0, height = 0;
    int numOfPixels;
    ...
    fgets(buff, 6, file); // read "P5" annotation
    fgets(buff, 6, file); // read image width
    width = atoi(buff);
    fgets(buff, 6, file); // read image height;
    height = atoi(buff);
    ...
    numOfPixels = height * width;
    for(int i = 0; i < NUM_OF_GROUPS; i++){
        freqs[i] = (double) counts[i] / numOfPixels;
    }
...
/*
    Prints relative frequencies of groups
*/
void printResults(double *freqs){
    int i;
    for(i = 0; i < NUM_OF_GROUPS; i++){
        printf("%d %f\n", i, freqs[i]);
    }
